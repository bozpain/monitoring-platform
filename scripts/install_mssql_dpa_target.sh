...
usage() {
  ...
  echo "  $TARGET_DIR/mssql-<target-name>.env.example"
}
...
target=$1
template="$TARGET_DIR/mssql-$target.env.example"
destination="$DPA_CONF_DIR/mssql-$target.env"
...
if [ ! -f "$template" ]; then
  echo "[ERROR] MSSQL DPA target template not found: $template"
  echo "[INFO] Set mssql_dpa=yes in inventory/targets.csv and run:"
...
echo "[DONE] MSSQL DPA target env installed: $destination"
echo ""
if grep -q "CHANGE_ME" "$destination"; then
  echo "[NEXT] Edit MSSQL credentials before starting sampler:"
  echo "       vi $destination"
  echo ""
fi

echo "[NEXT] Enable and test sampler:"
echo "       systemctl enable --now mssql_dpa_sampler@$target.timer"
echo "       systemctl start mssql_dpa_sampler@$target.service"
echo "       journalctl -u mssql_dpa_sampler@$target.service -n 100 --no-pager"
